...
export default async function Categorie({ params }: { params: { name: string , id: string } }){
    ...
    return(
        ...
        <Typography color="text.primary">{decodeURI(params.name)}</Typography>
      </Breadcrumbs>
         <div style={{display:"flex",justifyContent:"space-between",marginTop:"20px"}}>
         <a href={'/admin/categories/' + decodeURI(params.name) + '/' + params.id + '/edit'}>Edit Categorie</a>
         <a href={'/admin/categories/' + decodeURI(params.name) + '/' + params.id + '/new'}>Add Article</a>
         </div>
         {
          articles.length === 0 ? 
          <div>
          ...
          {articles.map((article,index)=>{
            return(
              <a
                  key={index}
                  href={"/admin/categories/" + decodeURI(params.name) + "/" + params.id + "/article/" + article.name + "/" + article.id }
                >
